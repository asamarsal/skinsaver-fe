...
export default function Navbar() {
  ...
  const navLinks = [
    { label: 'Selfie Scan', href: '/selfie-scan' },
    { label: 'Wishlist Audit', href: '/wishlist' },
    { label: 'Analysis', href: '/analysis' },
    { label: 'Premium Report', href: '/premium' },
    { label: 'ASP / MCP', href: '/mcp-demo' },
  ];

  return (
    <div className={`w-full sticky top-0 z-50 transition-all duration-300 ease-out ${isScrolled ? 'pt-0 px-0' : 'pt-3 px-[1%] sm:px-[2%] lg:px-[2.5%]'}`}>
      <nav className={`w-full bg-white/90 backdrop-blur-md transition-all duration-300 ease-out border border-gray-100 ${isScrolled ? 'max-w-full rounded-none border-x-0 border-t-0 shadow-none' : 'max-w-none rounded-[24px] shadow-sm'}`}>
        <div className={`h-[60px] flex items-center justify-between w-full mx-auto ${isScrolled ? 'max-w-[1640px] px-[1%] sm:px-[2%] lg:px-[2.5%]' : 'px-6 md:px-8'}`}>
          {/* Logo */}
            ...
            {navLinks.map(l => {
              ...
              return (
                <Link
                  key={l.label}
                  href={l.href}
                  className={`text-[13px] font-bold transition-colors relative py-1.5 ${
                    isActive ? 'text-pink-500 font-extrabold' : 'text-gray-800 hover:text-gray-900'
                  }`}
                >
                  {l.label}
                  {isActive && (
            ...
            {address ? (
              <div className="relative">
                <button
                  ...
                  className="h-9 px-4 inline-flex items-center justify-center gap-2 rounded-xl bg-gray-50 border border-gray-200 text-[13px] font-bold text-gray-700 hover:bg-gray-100 transition-all shadow-sm"
                >
              ...
              <button
                ...
                className="h-9 px-5 inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#e97db5] to-[#db5a9f] text-[13px] font-bold text-white hover:from-pink-500 hover:to-rose-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
              >
                <Wallet size={16} />
            ...
            {navLinks.map(l => {
              ...
              return (
                <Link
                  ...
                  className={`text-[15px] font-bold transition-colors py-2.5 border-b last:border-b-0 ${
                    isActive 
                      ? 'text-pink-500 font-extrabold border-b-pink-500' 
                      : 'text-gray-800 hover:text-gray-900 border-b-gray-50'
                  }`}
                >
                  {l.label}
                </Link>
